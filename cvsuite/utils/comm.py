...
_MISSING_LOCAL_PG_ERROR = (
    "Local process group is not yet created! Please use detectron2's "
    "`launch()` to start processes and initialize pytorch process group. "
    "If you need to start processes in other ways, please call "
    "comm.create_local_process_group(num_workers_per_machine) after calling "
...
def create_local_process_group(num_workers_per_machine: int) -> None:
    ...
    for i in range(num_machines):
        ranks_on_i = list(range(i * num_workers_per_machine,
...
def get_local_rank() -> int:
    """
    Returns:
        The rank of the current process within the local (per-machine)
        process group.
    """
    if not dist.is_available():
        return 0
...
def reduce_dict(input_dict: dict, average: bool = True):
    ...
    Args:
        input_dict: inputs to be reduced. All the values must be scalar
            CUDA Tensor.
